...
public class UnitSelection : MonoBehaviour {
    ...
    void Awake() {
        if (instance != null) {
            Debug.LogError("More than one UnitSelection instance in scene!");
            return;
        }
        instance = this;
    }
    ...
    private (Unit unit, float time) lastSelect;
    private float lastQuickSelectTime;

    public bool IsUnitSelected {  get { return selectedUnits.Count > 0; } }
    ...
    void Update() {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha1)) {
            Unit unit = GameMaster.instance.GetFirstUnitControlledByPlayerSlot(ConnectionManager.instance.MySlot);

            if (unit != null) {
                if (SelectedUnits.Count == 1 && SelectedUnits.Contains(unit)) {
                    if (Time.time - lastQuickSelectTime < 0.5) {
                        cam.GetComponent<CameraController>().FocusOn(unit.transform.position);
                    }
                }
        ...
        if (Input.GetMouseButtonDown(0)) {
            if (AbilityHandler.instance.IsAnythingQueued() || AbilityHandler.instance.AbilityJustUsed) {
                dragEligible = false;
                return;
            }
        ...
        if (Input.GetMouseButtonUp(0)) {
            if (AbilityHandler.instance.AbilityJustUsed || AbilityHandler.instance.IsAnythingQueued()) {
                return;
            }

            if (!dragSelect || !dragEligible) {
                ClickSelect(Input.mousePosition, Input.GetKey(KeyCode.LeftShift));
            } else {
                dragSelect = false;
                ...
                foreach (int _unitId in hoverSprites.Keys) {
                    ...
                    if (unit != null && ConnectionManager.instance.MySlot == unit.playerSlotNumber) {
                        SelectUnit(unit);
                    }
                }
    ...
    private void OnGUI() {
        if (dragEligible && dragSelect) {
            var rect = SelectionBox.GetScreenRect(dragStartPos, Input.mousePosition);
    ...
    private void OnTriggerStay(Collider other) {
        ...
        if (unit != null) {
            currentlyInBox.Add(unit.unitId); // see FixedUpdate() for explanation
            CreateHover(unit);
        }
    ...
    private void RaycastForHover(Vector2 _mousePos) {
        ...
        Unit unitHit = RaycastPreferNotSelected(ray, 300f);
    ...
    public Unit RaycastPreferNotSelected(Ray ray, float maxDistance) {
        RaycastHit[] hits = Physics.RaycastAll(ray, maxDistance, unitLayerMask);
        if (hits.Length == 0) {
            return null;
        }
        ...
        foreach (RaycastHit hit in hitList) {
            if (hit.distance < shortestDistance) {
                ...
                closestUnit = hit.collider.GetComponent<Unit>();
            }
        }
        ...
        if (Physics.Raycast(ray, out hit, 200, unitLayerMask)) {
            ...
            if (unit != null) {
                if (_shiftHeld) {
                    ...
                    lastSelect = (unit, Time.time); // strictly for double click LM selection purposes
                }
            }
        }
    }
    ...
    public void SelectUnit(Unit _unit) {
        ...
        if (!selectionSprites.ContainsKey(_unit.unitId)) {
            GameObject sprite = Instantiate(selectionSpritePrefab, _unit.transform);
    ...
    private void SelectAllLinemen() {
        List<Unit> linemen = GameMaster.instance.GetAllLinemenForPlayerSlot(ConnectionManager.instance.MySlot);

        if (linemen.Count > 0) {
            foreach (Unit LM in linemen) {
                SelectUnit(LM);
            }
        }
    }
    ...
    private void CreateHover(Unit _unit) {
        if (hoverSprites.ContainsKey(_unit.unitId)) {
            return;
        }

        hoverSprites.Add(_unit.unitId, Instantiate(hoverSpritePrefab, _unit.transform));
    }
